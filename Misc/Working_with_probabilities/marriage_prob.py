...
def two_sided_exponential_weight(x, a=5):
    if x >= 0.5:
        return (np.exp(a * (x - 0.5)) - 1) / (np.exp(a) - 1)
    else:
        return -(np.exp(-a * (x - 0.5)) - 1) / (np.exp(-a) - 1)
...
def calculate_probability(age, attractiveness, desperation_factor, age_weight=1, attr_weight=1, desp_weight=1):
    ...
    probability = base_probability * (weighted_attractiveness + weighted_desperation + diminishing_age_factor * age_weight)
    
    return probability

def likelihood(age, *args):
    attractiveness, wealth, beauty, gender, divorced, porn_factor, age_weight, attr_weight, desp_weight = args
...
def most_likely_age(attractiveness, wealth, beauty, gender='man', divorced=False, porn_factor=0, age_weight=1, attr_weight=1, desp_weight=1):
    result = minimize_scalar(likelihood, bounds=(20, 50), args=(attractiveness, wealth, beauty, gender, divorced, porn_factor, age_weight, attr_weight, desp_weight), method='bounded')
